...
const AllProduct = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await axiosSecure.get(
        `/gerProductPaginate?page=${currentPage}&limit=5`
      );
  ...
  return (
    <div className="font-primary lg:w-[1500px] md:w-[650px] w-[320px] mx-auto">
      <h1 className="md:text-4xl text-2xl font-semibold">All Products</h1>
      ...
      <div className="border border-gray-300 rounded-xl mt-5 py-5">
        <div className="w-[95%] mx-auto">
          ...
          {products?.map(
            (data, index) =>
              data?.authorData?.authorMail === email && (
                <div className="grid lg:grid-cols-6 md:grid-cols-4 grid-cols-1 gap-5 mt-4 items-center">
                  ...
                  <img
                    src={data?.authorData?.authorPhoto}
